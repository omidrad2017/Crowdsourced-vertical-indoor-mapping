...
smoothed_without_stairs(isnan(smoothed_without_stairs))=[];
 ...
 aggregated_increased_clusts = cell(1,K);
  for g = 1:K
   ...
   switch a
       case 1
       Aggness27_dataset_1_altitude_increased=sort(((power(ref_pressure./increased_smooth,1/5.257)-1)*(temps(a)+273.15))/0.0065,'ascend');
       figure
       plot (Aggness27_dataset_1_altitude_increased)
       title('Aggness27: altitude of dataset 1')
       case 2
       Aggness27_dataset_2_altitude_increased=sort(((power(ref_pressure./increased_smooth,1/5.257)-1)*(temps(a)+273.15))/0.0065,'ascend');
       figure
       plot (Aggness27_dataset_2_altitude_increased)
       title('Aggness27: altitude of dataset 2')
       case 3
       Aggness27_dataset_3_altitude_increased=sort(((power(ref_pressure./increased_smooth,1/5.257)-1)*(temps(a)+273.15))/0.0065,'ascend');
       plot (Aggness27_dataset_3_altitude_increased)
       title('Aggness27: altitude of dataset 3')
       case 4
       Aggness27_dataset_4_altitude_increased=sort(((power(ref_pressure./increased_smooth,1/5.257)-1)*(temps(a)+273.15))/0.0065,'ascend');
       plot (Aggness27_dataset_4_altitude_increased)
       title('Aggness27: altitude of dataset 4')
       case 5
       Aggness27_dataset_5_altitude_increased=sort(((power(ref_pressure./increased_smooth,1/5.257)-1)*(temps(a)+273.15))/0.0065,'ascend');
       plot (Aggness27_dataset_5_altitude_increased)
       title('Aggness27: altitude of dataset 5')
   end
  % Store the distortion percents of elbow method in a cell array to use
  % the array for final plot of elbows for each dataset
  distortions_Aggness27{a}=distorted;
  
end % end of main for loop

 % Aggregate altitude datasets into one aggregated altitude dataset
 if nummber_of_files == 5
   Aggness27_Aggregated_Altitudes = horzcat(Aggness27_dataset_1_altitude_increased,Aggness27_dataset_2_altitude_increased, Aggness27_dataset_3_altitude_increased,Aggness27_dataset_4_altitude_increased,Aggness27_dataset_5_altitude_increased);
 else % for this specific directory which has 4 datasets:
 Aggness27_Aggregated_Altitudes = horzcat(Aggness27_dataset_1_altitude_increased,Aggness27_dataset_2_altitude_increased, Aggness27_dataset_3_altitude_increased,Aggness27_dataset_4_altitude_increased);   
 end
 
figure
plot(distortions_Aggness27{1,1},'b*--')
title('Aggness27 elbows')
hold on
for j=2:length(file_names)
    plot(distortions_Aggness27{1,j},'b*--')
end
hold off


figure
plot(Aggness27_Aggregated_Altitudes)
title('Aggness27: aggregated altitudes')



%% Now we run elbow algorithm on 5 (or here 4)aggregated altitude datasets
[IDX2,C2,SUMD2,K2,distorted2]= best_kmeans_2(Aggness27_Aggregated_Altitudes');

% Finally the plot of  clustering result on aggregated altitude dataset
figure
plot(find(IDX2==1),Aggness27_Aggregated_Altitudes(IDX2==1),'*')
hold on
plot(find(IDX2==2),Aggness27_Aggregated_Altitudes(find(IDX2==2)),'*')
plot(find(IDX2==3),Aggness27_Aggregated_Altitudes(find(IDX2==3)),'*')
plot(find(IDX2==4),Aggness27_Aggregated_Altitudes(find(IDX2==4)),'*')
plot(find(IDX2==5),Aggness27_Aggregated_Altitudes(find(IDX2==5)),'*')
plot(find(IDX2==6),Aggness27_Aggregated_Altitudes(find(IDX2==6)),'*')
plot(find(IDX2==7),Aggness27_Aggregated_Altitudes(find(IDX2==7)),'*')
plot(Aggness27_Aggregated_Altitudes)

% To evaluate accurycy of each floor, you can use the resulting clusters of
% aggregated datasets

cluster_1 = Aggness27_Aggregated_Altitudes(IDX2==1);

cluster_2 = Aggness27_Aggregated_Altitudes(IDX2==2);

cluster_3 = Aggness27_Aggregated_Altitudes(IDX2==3);

cluster_4 = Aggness27_Aggregated_Altitudes(IDX2==4);

cluster_5 = Aggness27_Aggregated_Altitudes(IDX2==5);

cluster_6 = Aggness27_Aggregated_Altitudes(IDX2==6);

cluster_7 = Aggness27_Aggregated_Altitudes(IDX2==7);
